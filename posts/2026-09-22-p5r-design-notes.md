...
菜单项悬停时那块「红布铺开」的效果，用的是伪元素加 `scaleX`：
...
`transform-origin: left center` 是关键，它让色块从左边长出来，而不是从中间撑开。P5 里所有的动效都是这种从某一侧甩进来的节奏。
...
再叠一层斜条纹和一层噪点，画面就有那种印刷品的颗粒感。不干净，但很有劲。

## 页面转场

点站内链接的时候，会有五条斜向色块依次扫过把屏幕盖住。实现是五个不同延迟的 `div`：
...
延迟每条递增 55ms，就有了「甩过去」的视觉残留。整套转场 400ms 内结束，不会拖慢手感。

## 字体

正文用系统字体栈，中文不需要额外下载字体文件。标题用的是 Anton，它被压得很窄很挤，天然有海报字的感觉，只占 18KB，已经放进 `assets/fonts/`。

中文标题没有对应的免费粗斜体，我的做法是用 900 字重加合成斜体顶上：
...
上面那套是兜底版本。现在主流浏览器支持跨文档 View Transitions 了，所以那道斜向擦除直接写成 CSS 动画，一行 JS 都不需要：
...
老浏览器那边，`.wipe` 那套原样留着，然后用一次真实的特性探测决定走哪条路：
...
| `light-dark()` | 一套变量描述黑、纸白两种主题 | 前面先写 hex 兜底 |
| 容器查询 | 卡片网格按容器宽度自适应 | 回退到媒体查询 |
| `@starting-style` | 面板、弹层的入场 | 直接出现 |
| `content-visibility` | 屏幕外卡片跳过渲染 | 无影响 |

「先写 hex 再写 `light-dark()`」这个技巧可以单独说一下。CSS 里同一条声明写两遍，后一条不合法就自动保留前一条，等于免费拿到渐进增强：
...
这么改完，原来那整块 `html[data-theme='paper'] { ... }` 覆盖规则就全删了，`--fg`、`--line`、`--code-fg` 全都自动跟着变。
...
把 marked 和 highlight.js 下载到本地，是因为 GitHub Pages 在国内访问 `cdn.jsdelivr.net` 时好时坏。整站零外部请求，打开速度稳定。

## 怎么改成你自己的

最需要动的是这几处：

1. `assets/css/p5r.css` 顶部的 `:root`，换掉 `--red`，整站立刻就变成另一种气质（换成青蓝就是 Persona 3）
2. 各页面顶栏和页脚的站名文案，直接改 HTML
3. `assets/js/config.js`，建站日期（首页运行时间用）、首页显示篇数
