...
function walk(dir, out) {
  ...
  fs.readdirSync(dir).forEach((f) => {
    if (f === '.git' || f === 'node_modules' || f === 'tools') return;  // tools/ 是模板，不是产物
...
const posts = all.filter((f) => path.dirname(f) === path.join(ROOT, 'posts') && /\.html$/i.test(f));
...
/* ③ 文章索引与页面对应 */
const dataPath = path.join(ROOT, 'assets/js/posts-data.js');
let list = [];
if (!fs.existsSync(dataPath)) {
  fail('缺少 assets/js/posts-data.js（先运行 node tools/blog.js）');
} else {
  ...
  if (!list.length) fail('posts-data.js 里没有文章');
    ...
    const miss = list.filter((p) => !fs.existsSync(path.join(ROOT, p.url)));
...
[
  ...
  if (!fs.existsSync(path.join(ROOT, f))) { missAsset++; fail('缺少 ' + f); }
});
